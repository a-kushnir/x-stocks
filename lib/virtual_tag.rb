...
class VirtualTag
  ...
  ALL = [
    ...
    VirtualTag.new('Safe Dividend', 'Dividend Safety ≥ 4.0', ->(position) { position.stock.dividend_rating && position.stock.dividend_rating >= 4.0 }, ->(_user) { XStocks::AR::Stock.where('dividend_rating >= 4.0') }, 'fa-shield-alt', '21-desc'),
    VirtualTag.new('High Score', 'Score > 60', ->(position) { position.stock.metascore && position.stock.metascore > 60 }, ->(_user) { XStocks::AR::Stock.where('metascore > 60') }, 'fa-shield-alt', '23-desc'),
    ...
    VirtualTag.new('Ex Date Soon', 'Ex Date ≤ 1 month in the future', ->(position) { XStocks::Stock.new(position.stock).next_month_ex_date? }, ->(_user) { XStocks::AR::Stock.where(['next_div_ex_date > ? and next_div_ex_date <= ?', XStocks::Stock::Dividends.future_ex_date, XStocks::Stock::Dividends.next_month_ex_date]) }, 'fa-calendar-alt', '22-asc'),
