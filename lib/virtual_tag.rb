...
class VirtualTag
  ...
  ALL = [
    ...
    VirtualTag.new('Safe Dividend', 'Dividend Safety ≥ 4.0', ->(position) { position.stock.dividend_rating && position.stock.dividend_rating >= 4.0 }, ->(_user) { XStocks::AR::Stock.where('dividend_rating >= 4.0') }, 'fa-shield-alt', '10-desc'),
    VirtualTag.new('High Score', 'Score > 60', ->(position) { position.stock.metascore && position.stock.metascore > 60 }, ->(_user) { XStocks::AR::Stock.where('metascore > 60') }, 'fa-shield-alt', '12-desc'),
    ...
    VirtualTag.new('Ex Date Soon', 'Ex Date ≤ 1 month in the future', ->(position) { position.stock.next_div_ex_date && position.stock.next_div_ex_date > Date.today && position.stock.next_div_ex_date < 1.month.since }, ->(_user) { XStocks::AR::Stock.where(['next_div_ex_date > ? and next_div_ex_date < ?', Date.today, 1.month.since]) }, 'fa-calendar-alt', '11-asc'),
