...
class VirtualTag
  ...
  ALL = [
    ...
    VirtualTag.new('Ex Date Soon', 'Ex Date ≤ 1 month in the future', ->(position) { XStocks::Stock.new(position.stock).next_month_ex_date? }, ->(_user) { XStocks::AR::Stock.where(['next_div_ex_date > ? and next_div_ex_date <= ?', XStocks::Stock::Dividends.future_ex_date, XStocks::Stock::Dividends.next_month_ex_date]) }, 'fa-calendar-alt', '11-asc'),
